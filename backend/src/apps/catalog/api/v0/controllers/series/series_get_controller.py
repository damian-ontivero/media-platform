...
from src.contexts.catalog.series.application.query import SerieCountQuery, SerieSearchByCriteriaQuery
...
class SeriesGetController(Controller):
    ...
    def run(self, criteria: str | None) -> Response:
        ...
        series = self._query_bus.ask(SerieSearchByCriteriaQuery(**criteria))
        total = self._query_bus.ask(SerieCountQuery())
