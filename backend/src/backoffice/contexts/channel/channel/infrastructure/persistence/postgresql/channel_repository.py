...
class PostgreSQLChannelRepository(ChannelRepository):
    ...
    def search_all(self) -> list[Channel]:
        with self._session() as session:
            ...
            return [
                Channel.from_primitives(channel_db.id, channel_db.name, channel_db.image) for channel_db in channels_db
            ]
    ...
    def search(self, id: str) -> Channel | None:
        with self._session() as session:
            ...
            if channel_db is not None:
                return Channel.from_primitives(channel_db.id, channel_db.name, channel_db.image)

    def matching(self, criteria: Criteria) -> list[Channel]:
        with self._session() as session:
            query = session.query(PostgreSQLChannelModel)
            query = criteria_to_sqlalchemy_query(query, PostgreSQLChannelModel, criteria)
            channels_db = query.all()
            return [
                Channel.from_primitives(channel_db.id, channel_db.name, channel_db.image) for channel_db in channels_db
            ]
