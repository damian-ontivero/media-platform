from src.backoffice.contexts.shared.domain import AggregateRoot, EntityId
...
class Content(AggregateRoot):
    def __init__(
        self, id: EntityId, files: list[ContentFile], rating: float, metadata_: ContentMetadata, channel_id: EntityId
    ) -> None:
        self._id = id
        self._files = files
        self._rating = rating
    ...
    @classmethod
    def create(cls, files: list[dict], rating: float, metadata_: dict, channel_id: str) -> "Content":
        return cls(
            EntityId.generate(),
    ...
    @classmethod
    def from_primitives(cls, id: str, files: list[dict], rating: float, metadata_: dict, channel_id: str) -> "Content":
        return cls(
            EntityId(id),
            [ContentFile(**file) for file in files],
            rating,
    ...
    def update(self, files: list[dict], rating: float, metadata_: dict, channel_id: str) -> None:
    ...
    def __repr__(self) -> str:
        return (
            "{c}(id={id!r}, files={files!r}, rating={rating!r}, " "metadata_={metadata_!r} channel_id={channel_id!r})"
        ).format(
