...
class ChannelGetSchema(BaseModel):
    id: str = Field(..., description="Channel ID", examples=["123e4567-e89b-12d3-a456-426614174000"])
    title: str = Field(..., description="Channel Title", examples=["Channel Title"])
    languages: list[str] = Field(..., description="Languages", examples=[["en", "es"]])
    image: ImageSchema = Field(..., description="image", examples=[{"path": "image.jpg"}])
