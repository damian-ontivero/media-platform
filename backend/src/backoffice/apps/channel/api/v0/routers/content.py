...
from ..schemas import ContentCreateSchema, ContentGetSchema, ContentPaginatedResponseSchema, ContentUpdateSchema

router = APIRouter(prefix="/content", tags=["Content"])


@router.get(
    "", response_model=ContentPaginatedResponseSchema, status_code=status.HTTP_200_OK, description="Search Content"
)
async def search(
    request: Request,
    criteria: Annotated[
...
@router.get("/{id}", response_model=ContentGetSchema, status_code=status.HTTP_200_OK, description="Find Content")
async def find(
    request: Request,
    id: Annotated[str, Path(..., description="Id of the Content", example="123e4567-e89b-12d3-a456-426614174000")],
):
    controller = container.get("ContentGetController")
    return controller.run(request)


@router.get("/{id}/file", status_code=status.HTTP_200_OK, description="Find Content File")
...
@router.post("", status_code=status.HTTP_201_CREATED, description="Create Content")
async def create(
    request: Request,
...
@router.put("/{id}", status_code=status.HTTP_200_OK, description="Update Content")
async def update(
    request: Request,
    id: Annotated[str, Path(..., description="Id of the Content", example="123e4567-e89b-12d3-a456-426614174000")],
...
@router.delete("/{id}", status_code=status.HTTP_200_OK, description="Delete Content")
async def delete(
    request: Request,
    id: Annotated[str, Path(..., description="Id of the Content", example="123e4567-e89b-12d3-a456-426614174000")],
):
