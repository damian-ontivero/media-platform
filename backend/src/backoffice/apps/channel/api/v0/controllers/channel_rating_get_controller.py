...
class ChannelRatingGetController(Controller):
    def __init__(self, channel_finder: ChannelFinder, content_searcher: ContentSearcher) -> None:
    ...
    def run(self, request: Request) -> Response:
        ...
        criteria = {
            "filter": {
                "conjunction": "AND",
                "conditions": [{"field": "channel_id", "operator": "EQUALS", "value": channel.get_id()}],
            },
            "sort": [],
            "page_size": 15,
            "page_number": 1,
        ...
        return Response(
            content=str(round(rating_avg, 2)), status_code=status.HTTP_200_OK, media_type="application/json"
        )
